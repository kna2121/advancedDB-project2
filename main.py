...
RELATION_PROMPTS = {
    1: "Identify any found school attended relationships in the sentence below. "
       ...
       "Return output in the format Person ; School.",
    2: "Identify any found work-for relationships in the sentence below. "
       "Return only if the subject is a person and the object is a company. "
       "Return output in the format Person ; Place of work.",
    3: "Identify any live-in relationships in the sentence below. "
       "Return only if the subject is a person and the object is a city, state, or country. "
       "Return output in the format Person ; Location.",
    4: "Identify any top-member employee relationships in the sentence below. "
       "Return only if the subject is an ORGANIZATION and the object is a PERSON. "
       "Return output in the format Organization ; Person."
}
...
def get_gemini(prompt, model_name="gemini-2.0-flash", max_tokens=100, temperature=0.2, top_p=1, top_k=32):
    time.sleep(5)
    model = genai.GenerativeModel(model_name)
    config = genai.types.GenerationConfig(max_output_tokens=max_tokens, temperature=temperature, top_p=top_p, top_k=top_k)
...
def main():
    ...
    # === Load SpanBERT if needed ===
    if method == "spanbert":
        print("Loading SpanBERT model fine-tuned on TACRED...")
        tokenizer = AutoTokenizer.from_pretrained("SpanBERT/spanbert-large-cased-finetuned-tacred")
        model = AutoModelForSequenceClassification.from_pretrained("SpanBERT/spanbert-large-cased-finetuned-tacred")
        model.eval()
    ...
    while len(X) < k:
        print(f"\n=========== Iteration: {iterations} - Query: {current_query} ===========")
        ...
        for i, url in enumerate(urls):
            if url in processed:
                continue
            print(f"\nURL ({i + 1}/{len(urls)}): {url}")
            text = fetch_page_text(url)
            print(f"\tWebpage length: {len(text)} characters")

            doc = nlp(text)
            sentences = list(doc.sents)
            print(f"\t{len(sentences)} sentences found. Processing...")
            ...
            for idx, sent in enumerate(sentences):
                ...
                if method == "gemini":
                    prompt = f"{RELATION_PROMPTS[r]} Sentence: {sent.text.strip()}"
                    try:
                        response = get_gemini(prompt)
                    except Exception as e:
                    ...
                    if ";" in response:
                        ...
                        for line in lines:
                            if ";" not in line:
                                continue
                            try:
                                subj, obj = map(str.strip, line.split(";", 1))
                            except ValueError:
                                continue
                            if (subj, obj, 1.0) not in X:
                                print(f"\t✔ Extracted: {subj} → {obj}")
                                X.add((subj, obj, 1.0))

                elif method == "spanbert":
                    # ==== TODO: Add spanbert relation classification here ====
                    pass
    ...
    for subj, obj, conf in list(X)[:k]:
        print(f"Subject: {subj}\tObject: {obj}\tConfidence: {conf:.2f}")
    print(f"\nTotal ISE iterations: {iterations}")
